-module(exqwalizer_erl_parse_transform).
-export([
    parse_transform/2,
...
variable_list([{var,Anno,Var}|Vs]) ->
    [{var,strip_anno(Anno),Var}|variable_list(Vs)];
variable_list([]) -> [].
...
record_defs([{record_field,Anno,{atom,Aa,A},Val0}|Is]) ->
    Val1 = expr(Val0),
    [{record_field,strip_anno(Anno),{atom,Aa,A},Val1}|record_defs(Is)];
record_defs([{record_field,Anno,{atom,Aa,A}}|Is]) ->
    [{record_field,strip_anno(Anno),{atom,Aa,A}}|record_defs(Is)];
record_defs([{typed_record_field,{record_field,Anno,{atom,Aa,A},Val0},Type}|
             Is]) ->
    Val1 = expr(Val0),
    Type1 = type(Type),
    [{typed_record_field,{record_field,strip_anno(Anno),{atom,Aa,A},Val1},Type1}|
     record_defs(Is)];
record_defs([{typed_record_field,{record_field,Anno,{atom,Aa,A}},Type}|Is]) ->
    Type1 = type(Type),
    [{typed_record_field,{record_field,strip_anno(Anno),{atom,Aa,A}},Type1}|
     record_defs(Is)];
record_defs([]) -> [].
...
clause({clause,Anno,H0,G0,B0}) ->
    ...
    G1 = guard(G0),
    B1 = exprs(B0),
    {clause,strip_anno(Anno),H1,G1,B1}.
...
pattern({var,Anno,V}) -> {var,strip_anno(Anno),V};
pattern({match,Anno,L0,R0}) ->
    L1 = pattern(L0),
    R1 = pattern(R0),
    {match,strip_anno(Anno),L1,R1};
pattern({integer,Anno,I}) -> {integer,strip_anno(Anno),I};
pattern({char,Anno,C}) -> {char,strip_anno(Anno),C};
pattern({float,Anno,F}) -> {float,strip_anno(Anno),F};
pattern({atom,Anno,A}) -> {atom,strip_anno(Anno),A};
pattern({string,Anno,S}) -> {string,strip_anno(Anno),S};
pattern({nil,Anno}) -> {nil,strip_anno(Anno)};
pattern({cons,Anno,H0,T0}) ->
    H1 = pattern(H0),
    T1 = pattern(T0),
    {cons,strip_anno(Anno),H1,T1};
pattern({tuple,Anno,Ps0}) ->
    Ps1 = pattern_list(Ps0),
    {tuple,strip_anno(Anno),Ps1};
pattern({map,Anno,Ps0}) ->
    Ps1 = pattern_list(Ps0),
    {map,strip_anno(Anno),Ps1};
pattern({map_field_exact,Anno,K,V}) ->
    Ke = expr(K),
    Ve = pattern(V),
    {map_field_exact,strip_anno(Anno),Ke,Ve};
pattern({record,Anno,Name,Pfs0}) ->
    Pfs1 = pattern_fields(Pfs0),
    {record,strip_anno(Anno),Name,Pfs1};
pattern({record_index,Anno,Name,Field0}) ->
    Field1 = pattern(Field0),
    {record_index,strip_anno(Anno),Name,Field1};
pattern({record_field,Anno,Rec0,Name,Field0}) ->
    Rec1 = expr(Rec0),
    Field1 = expr(Field0),
    {record_field,strip_anno(Anno),Rec1,Name,Field1};
pattern({record_field,Anno,Rec0,Field0}) ->
    Rec1 = expr(Rec0),
    Field1 = expr(Field0),
    {record_field,strip_anno(Anno),Rec1,Field1};
pattern({bin,Anno,Fs}) ->
    Fs2 = pattern_grp(Fs),
    {bin,strip_anno(Anno),Fs2};
pattern({op,Anno,Op,A}) ->
    {op,strip_anno(Anno),Op,A};
pattern({op,Anno,Op,L,R}) ->
    {op,strip_anno(Anno),Op,L,R}.
...
pattern_grp([{bin_element,Anno,E1,S1,T1} | Fs]) ->
    ...
    [{bin_element,strip_anno(Anno),expr(E1),S2,T2} | pattern_grp(Fs)];
pattern_grp([]) ->
    [].
...
pattern_fields([{record_field,Af,{var,Aa,'_'},P0}|Pfs]) ->
    P1 = pattern(P0),
    [{record_field,Af,{var,strip_anno(Aa),'_'},P1}|pattern_fields(Pfs)];
pattern_fields([]) -> [].
...
guard_test(Expr={call,Anno,{atom,Aa,F},As0}) ->
  ...
  true ->
      As1 = gexpr_list(As0),
      {call,strip_anno(Anno),{atom,Aa,F},As1};
  _ ->
      gexpr(Expr)
    end;
guard_test(Any) ->
...
gexpr({var,Anno,V}) -> {var,strip_anno(Anno),V};
gexpr({integer,Anno,I}) -> {integer,strip_anno(Anno),I};
gexpr({char,Anno,C}) -> {char,strip_anno(Anno),C};
gexpr({float,Anno,F}) -> {float,strip_anno(Anno),F};
gexpr({atom,Anno,A}) -> {atom,strip_anno(Anno),A};
gexpr({string,Anno,S}) -> {string,strip_anno(Anno),S};
gexpr({nil,Anno}) -> {nil,strip_anno(Anno)};
gexpr({map,Anno,Map0,Es0}) ->
    [Map1|Es1] = gexpr_list([Map0|Es0]),
    {map,strip_anno(Anno),Map1,Es1};
gexpr({map,Anno,Es0}) ->
    Es1 = gexpr_list(Es0),
    {map,strip_anno(Anno),Es1};
gexpr({map_field_assoc,Anno,K,V}) ->
    Ke = gexpr(K),
    Ve = gexpr(V),
    {map_field_assoc,strip_anno(Anno),Ke,Ve};
...
gexpr({record,Anno,Name,Inits0}) ->
    ...
    {record,strip_anno(Anno),Name,Inits1};
...
gexpr({call,Anno,{remote,Aa,{atom,Ab,erlang},{atom,Ac,F}},As0}) ->
  ...
  true -> As1 = gexpr_list(As0),
    {call,strip_anno(Anno),{remote,strip_anno(Aa),{atom,strip_anno(Ab),erlang},{atom,strip_anno(Ac),F}},As1}
    end;
gexpr({bin,Anno,Fs}) ->
    Fs2 = pattern_grp(Fs),
    {bin,strip_anno(Anno),Fs2};
...
expr({var,Anno,V}) -> {var,strip_anno(Anno),V};
expr({integer,Anno,I}) -> {integer,strip_anno(Anno),I};
expr({float,Anno,F}) -> {float,strip_anno(Anno),F};
expr({atom,Anno,A}) -> {atom,strip_anno(Anno),A};
expr({string,Anno,S}) -> {string,strip_anno(Anno),S};
expr({char,Anno,C}) -> {char,strip_anno(Anno),C};
expr({nil,Anno}) -> {nil,strip_anno(Anno)};
...
expr({lc,Anno,E0,Qs0}) ->
    ...
    E1 = expr(E0),
    {lc,strip_anno(Anno),E1,Qs1};
expr({bc,Anno,E0,Qs0}) ->
    Qs1 = comprehension_quals(Qs0),
    E1 = expr(E0),
    {bc,strip_anno(Anno),E1,Qs1};
expr({mc,Anno,E0,Qs0}) ->
    Qs1 = comprehension_quals(Qs0),
    E1 = expr(E0),
    {mc,strip_anno(Anno),E1,Qs1};
expr({tuple,Anno,Es0}) ->
    Es1 = expr_list(Es0),
    {tuple,strip_anno(Anno),Es1};
...
expr({record,Anno,Rec0,Name,Upds0}) ->
    ...
    {record,strip_anno(Anno),Rec1,Name,Upds1};
...
expr({block,Anno,Es0}) ->
    ...
    Es1 = exprs(Es0),
    {block,strip_anno(Anno),Es1};
expr({'if',Anno,Cs0}) ->
    Cs1 = icr_clauses(Cs0),
    {'if',strip_anno(Anno),Cs1};
expr({'case',Anno,E0,Cs0}) ->
    E1 = expr(E0),
    Cs1 = icr_clauses(Cs0),
    {'case',strip_anno(Anno),E1,Cs1};
expr({'receive',Anno,Cs0}) ->
    Cs1 = icr_clauses(Cs0),
    {'receive',strip_anno(Anno),Cs1};
expr({'receive',Anno,Cs0,To0,ToEs0}) ->
    To1 = expr(To0),
    ToEs1 = exprs(ToEs0),
    Cs1 = icr_clauses(Cs0),
    {'receive',strip_anno(Anno),Cs1,To1,ToEs1};
expr({'try',Anno,Es0,Scs0,Ccs0,As0}) ->
    Es1 = exprs(Es0),
    Scs1 = icr_clauses(Scs0),
    Ccs1 = icr_clauses(Ccs0),
    As1 = exprs(As0),
    {'try',strip_anno(Anno),Es1,Scs1,Ccs1,As1};
expr({'fun',Anno,Body}) ->
    case Body of
  {clauses,Cs0} ->
      Cs1 = fun_clauses(Cs0),
      {'fun',strip_anno(Anno),{clauses,Cs1}};
  {function,F,A} ->
      {'fun',strip_anno(Anno),{function,F,A}};
  {function,M0,F0,A0} ->
      M = expr(M0),
      F = expr(F0),
      A = expr(A0),
      {'fun',strip_anno(Anno),{function,M,F,A}}
    end;
expr({named_fun,Anno,Name,Cs}) ->
    {named_fun,strip_anno(Anno),Name,fun_clauses(Cs)};
expr({call,Anno,F0,As0}) ->
    ...
    F1 = expr(F0),
    As1 = expr_list(As0),
    {call,strip_anno(Anno),F1,As1};
expr({'catch',Anno,E0}) ->
    %% No new variables added.
    E1 = expr(E0),
    {'catch',strip_anno(Anno),E1};
expr({'maybe',MaybeAnno,Es0}) ->
    Es = exprs(Es0),
    {'maybe',strip_anno(MaybeAnno),Es};
expr({'maybe',MaybeAnno,Es0,{'else',ElseAnno,Cs0}}) ->
    Es = exprs(Es0),
    Cs = clauses(Cs0),
    {'maybe',strip_anno(MaybeAnno),Es,{'else',ElseAnno,Cs}};
expr({maybe_match,Anno,P0,E0}) ->
    E = expr(E0),
    P = pattern(P0),
    {maybe_match,strip_anno(Anno),P,E};
expr({match,Anno,P0,E0}) ->
    E1 = expr(E0),
    P1 = pattern(P0),
    {match,strip_anno(Anno),P1,E1};
expr({bin,Anno,Fs}) ->
    Fs2 = pattern_grp(Fs),
    {bin,strip_anno(Anno),Fs2};
expr({op,Anno,Op,A0}) ->
...
record_inits([{record_field,Af,{var,Aa,'_'},Val0}|Is]) ->
    Val1 = expr(Val0),
    [{record_field,Af,{var,strip_anno(Aa),'_'},Val1}|record_inits(Is)];
record_inits([]) -> [].
...
comprehension_quals([{generate,Anno,P0,E0}|Qs]) ->
    E1 = expr(E0),
    P1 = pattern(P0),
    [{generate,strip_anno(Anno),P1,E1}|comprehension_quals(Qs)];
comprehension_quals([{b_generate,Anno,P0,E0}|Qs]) ->
    E1 = expr(E0),
    P1 = pattern(P0),
    [{b_generate,strip_anno(Anno),P1,E1}|comprehension_quals(Qs)];
comprehension_quals([{m_generate,Anno,P0,E0}|Qs]) ->
    E1 = expr(E0),
    P1 = pattern(P0),
    [{m_generate,strip_anno(Anno),P1,E1}|comprehension_quals(Qs)];
...
function_type_list([{type,Anno,bounded_fun,[Ft,Fc]}|Fts]) ->
    ...
    [{type,strip_anno(Anno),bounded_fun,[Ft1,Fc1]}|function_type_list(Fts)];
...
function_type({type,Anno,'fun',[{type,At,product,As},B]}) ->
    As1 = type_list(As),
    B1 = type(B),
    {type,strip_anno(Anno),'fun',[{type,At,product,As1},B1]}.
...
constraint({type,Anno,constraint,[{atom,Annoa,A},[V,T]]}) ->
    V1 = type(V),
    T1 = type(T),
    {type,strip_anno(Anno),constraint,[{atom,strip_anno(Annoa),A},[V1,T1]]}.

type({ann_type,Anno,[{var,Av,V},T]}) ->
    T1 = type(T),
    {ann_type,strip_anno(Anno),[{var,strip_anno(Av),V},T1]};
type({atom,Anno,A}) ->
    {atom,strip_anno(Anno),A};
type({integer,Anno,I}) ->
    {integer,strip_anno(Anno),I};
type({char,Anno,C}) ->
    {char,strip_anno(Anno),C};
type({op,Anno,Op,T}) ->
    T1 = type(T),
    {op,strip_anno(Anno),Op,T1};
type({op,Anno,Op,L,R}) ->
    L1 = type(L),
    R1 = type(R),
    {op,strip_anno(Anno),Op,L1,R1};
type({type,Anno,binary,[M,N]}) ->
    M1 = type(M),
    N1 = type(N),
    {type,strip_anno(Anno),binary,[M1,N1]};
type({type,Anno,'fun',[]}) ->
    {type,strip_anno(Anno),'fun',[]};
type({type,Anno,'fun',[{type,At,any},B]}) ->
    B1 = type(B),
    {type,strip_anno(Anno),'fun',[{type,At,any},B1]};
type({type,Anno,range,[L,H]}) ->
    L1 = type(L),
    H1 = type(H),
    {type,strip_anno(Anno),range,[L1,H1]};
type({type,Anno,map,any}) ->
    {type,strip_anno(Anno),map,any};
type({type,Anno,map,Ps}) ->
    Ps1 = map_pair_types(Ps),
    {type,strip_anno(Anno),map,Ps1};
type({type,Anno,record,[{atom,Aa,N}|Fs]}) ->
    Fs1 = field_types(Fs),
    {type,strip_anno(Anno),record,[{atom,Aa,N}|Fs1]};
type({remote_type,Anno,[{atom,Am,M},{atom,An,N},As]}) ->
    As1 = type_list(As),
    {remote_type,strip_anno(Anno),[{atom,Am,M},{atom,An,N},As1]};
type({type,Anno,tuple,any}) ->
    {type,strip_anno(Anno),tuple,any};
type({type,Anno,tuple,Ts}) ->
    Ts1 = type_list(Ts),
    {type,strip_anno(Anno),tuple,Ts1};
type({type,Anno,union,Ts}) ->
    Ts1 = type_list(Ts),
    {type,strip_anno(Anno),union,Ts1};
type({var,Anno,V}) ->
    {var,strip_anno(Anno),V};
type({user_type,Anno,N,As}) ->
    As1 = type_list(As),
    {user_type,strip_anno(Anno),N,As1};
type({type,Anno,N,As}) ->
    As1 = type_list(As),
    {type,strip_anno(Anno),N,As1}.

map_pair_types([{type,Anno,map_field_assoc,[K,V]}|Ps]) ->
    K1 = type(K),
    V1 = type(V),
    [{type,strip_anno(Anno),map_field_assoc,[K1,V1]}|map_pair_types(Ps)];
map_pair_types([{type,Anno,map_field_exact,[K,V]}|Ps]) ->
    K1 = type(K),
    V1 = type(V),
    [{type,strip_anno(Anno),map_field_exact,[K1,V1]}|map_pair_types(Ps)];
map_pair_types([]) -> [].

field_types([{type,Anno,field_type,[{atom,Aa,A},T]}|Fs]) ->
    T1 = type(T),
    [{type,strip_anno(Anno),field_type,[{atom,Aa,A},T1]}|field_types(Fs)];
field_types([]) -> [].
...
strip_anno(Anno) when is_list(Anno) ->
    proplists:get_value(location, Anno, 0);
strip_anno(Anno) -> Anno.
